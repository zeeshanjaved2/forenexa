...
class PaymentController extends Controller
{
    ...
    public static function insertDeposit($gateway, $plan, $depositAmount)
    {

        $user = auth()->user();
        $charge = $gateway->fixed_charge + ($depositAmount * $gateway->percent_charge / 100);
        $payable = $depositAmount + $charge;
    ...
    public static function userDataUpdate($deposit, $isManual = null)
    {
        ...
        if ($deposit->status == Status::PAYMENT_INITIATE || $deposit->status == Status::PAYMENT_PENDING) {
            ...
            if ($deposit->plan_id) {
                $user->total_investment += $deposit->amount;
            }
            $user->save();
            ...
            if ($deposit->plan_id) {
                ...
                $investment->max_earning  = $eligiblePlan->max_earning * $deposit->amount;
                $investment->save();
                ...
                $transaction->trx_type     = '-';
                $transaction->details      = 'Subscribe ' . $eligiblePlan->name . ' Plan';
                $transaction->trx          = $trx;
                $transaction->remark       = 'subscribe_plan';
                $transaction->save();

                levelCommission($user, $deposit->amount, 'plan_subscribe_commission', $trx);
