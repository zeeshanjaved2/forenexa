...
class LanguageController extends Controller
{
    ...
    public function langStore(Request $request)
    {
        $request->validate([
            ...
            'flag' =>  ['required', new FileTypeValidate(['jpg', 'jpeg', 'png'])]
        ]);
        ...
        if ($request->hasFile('flag')) {
            try {
                $path = getFilePath('flag');
                $size = getFileSize('flag');
                $language->flag = fileUploader($request->flag, $path, $size, $language->flag);
            } catch (\Exception $exp) {
                return response()->json([
                    'status' => false,
                    'error' => 'Couldn\'t upload flag'
                ]);
            }
        }
    ...
    public function langUpdate(Request $request, $id)
    {
        $request->validate([
            'name' => 'required',
            'flag' =>  [new FileTypeValidate(['jpg', 'jpeg', 'png'])]
        ]);
    ...
    public function langDelete($id)
    {
        ...
        unlink((getFilePath('flag').'/'.$lang->flag));
        $lang->delete();
