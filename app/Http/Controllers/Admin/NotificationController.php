...
class NotificationController extends Controller
{
    ...
    public function UserNotiGet()
    {

        $pageTitle = 'User Notification Modal';
        $general = gs();
        // dd($general);
        return view('admin.notification.user_notification', compact('pageTitle', 'general'));
    }

    public function UserNotiStore(Request $request)
    {
        // dd($request->all());
        $general = gs();
        $general->notification_modal = $request->notification_modal == 'on' ? 1 : 0;
        $general->modal_text = $request->modal_text;
        $general->save();
