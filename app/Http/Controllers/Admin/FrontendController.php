...
class FrontendController extends Controller
{
        ...
        public function templates()
    {
        ...
        foreach ($temPaths as $key => $temp) {
            $relativePath = Str::after($temp, 'views/');
            $CurrentPath = $baseUrl . '/assets/' . $relativePath . '/preview.jpg';
            $arr                      = explode('/', $temp);
            $tempname                 = end($arr);
            $templates[$key]['name']  = $tempname;
            $templates[$key]['image'] = $CurrentPath;
        }
    ...
    // {
    //     $pageTitle = 'Templates';
    //     $templatePaths = array_filter(glob(resource_path('views/templates/*')), 'is_dir');
    //     dd($templatePaths);
    //     $templates = [];
    //     foreach ($templatePaths as $key => $path) {
    //         $templateName = basename($path);
    //         dd($path);
    //         $templates[$key]['name']  = $templateName;
    //         $templates[$key]['image'] = asset($path) . '/preview.jpg';
    //     }
    //     $extraTemplates = json_decode(getTemplates(), true);
    ...
    public function frontendSections($key)
    {
        ...
        $content   = Frontend::where('data_keys', $key . '.content')->where('tempname', $general->active_template)->orderBy('id', 'desc')->first();
        $elements  = Frontend::where('data_keys', $key . '.element')->where('tempname', $general->active_template)->orderBy('id')->orderBy('id', 'desc')->get();
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        if ($request->id) {
            ...
            if($key=='seo'){
                ...
                $content = Frontend::where('tempname', gs()->active_template)->where('data_keys', $key . '.' . $request->type)->first();
            }
        ...
        if ($content->data_keys != 'seo.data') {
            $content->tempname = gs()->active_template;
        }
        $content->save();
