                        ...
                        <div class="price-item section-bg">
                            ...
                            <div class="price-item__footer">
                                <ul class="price-item__list">
                                    <li><i class="far fa-check-circle {{ $class }}"></i> <span>@lang('Investment Range') :
                                            {{ getAmount($plan->min_price) }}{{ __($general->cur_sym) }}-{{ getAmount($plan->max_price) }}{{ __($general->cur_sym) }}
                                    </li>
                                    <li><i class="far fa-check-circle {{ $class }}"></i> <span>@lang('Non-Working Monthly Profit') :
                                            {{ $plan->monthly_profit }}%</span></li>
                                    @if ($general->deposit_commission == 1)
                                        <li><i class="far fa-check-circle {{ $class }}"></i>
                                            <span>@lang('Ref Deposit Commission') : {{ getAmount(depositCommissionLast()) }}%</span>
                                        </li>
                                    @endif

                                    @if ($general->ptc_view_commission == 1)
                                        <li><i class="far fa-check-circle {{ $class }}"></i>
                                            <span>@lang('Ref Earning Commission') : {{ getAmount(earningCommissionLast()) }}%</span>
                                        </li>
                                    @endif

                                    @if ($general->ptc_view_commission == 1)
                                    <li><i class="far fa-check-circle {{ $class }}"></i>
                                        <span>@lang('Ref Plan Upgrade Commission') : {{ getAmount(planCommissionLast()) }}%</span>
                                    </li>
                                @endif
                        ...
                        <div class="price-item section-bg">
                            ...
                            <div class="price-item__footer">
                                <ul class="price-item__list">
                                    ...
                                    @if (auth()->check())
                                        <li>
                                            <i class="far fa-check-circle {{ $class }}"></i>
                                            <span>
                                                @lang('Your total investment') :
                                                @if (auth()->user()->plan_id == $plan->id)
                                                    {{ getAmount(auth()->user()->total_investment) }}
                                                @else
                                                    0
                                                @endif
                                                {{ __($general->cur_sym) }}
        ...
        (function($) {
            ...
            $('.buyBtn').click(function() {
                ...
                modal.find('.dailyLimit').html(
                    `Monthly Earning : <span class="text--danger"> ${plan.monthly_profit}%</span>`)
                modal.find('.refLevel').html(
                    `Referral Level: <span class="text--danger">${plan.ref_level} </span>`)
                modal.find('.validity').html(
                    `Total Earning: <span class="text--danger"> ${plan.max_earning}X </span>`)
